...
- `shared/commonMain` — pure Kotlin: domain models, service **interfaces**
  ...
  the `QSenseLogo` (drawn in Compose Canvas), and `AppContainer` (constructor-injected class).
...
- `androidApp` — `QSenseApplication` builds the container once; `MainActivity` passes it +
  ...
  mark (navy bg + white waveform + coral dot) in `res/drawable*/ic_launcher_*`.
...
- **RAG grounding**: `InMemoryKnowledgeBase` (commonMain, pure keyword lookup on part name — no
  embeddings) returns known symptom→cause→fix entries; `GenerateDiagnosisUseCase` injects them into
  the prompt. GenieX still generates the diagnosis — the KB only grounds it.
- **MQTT** (`com.hivemq:hivemq-mqtt-client:1.3.15`). MQTT 5, QoS 1. Namespaced topics
  `qsense/{namespace}/alerts` and `.../resolutions` (default namespace `qsense-demo`). Clean
  start + automatic reconnect + explicit resubscribe on every connect. Resolutions are marked
  locally resolved only after PUBACK. Requires the `INTERNET` permission + Netty packaging
  excludes in `androidApp/build.gradle.kts`.
- **Serialization**: kotlinx-serialization (runtime 1.11.0). `JsonProviders.strict` for MQTT
  payloads, `JsonProviders.lenient` for LLM output. UTF-8 both directions.
